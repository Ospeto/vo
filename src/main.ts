...
} from "./services/hotkey-service.js";
import {
...
} from "./services/native-paste-addon.js";
import {
...
} from "./services/recording-error.js";
import {
...
export async function handleSignalTermination(signal: string) {
	...
	try {
		...
		logger.error(
			{ err: shutdownErr?.message || String(shutdownErr), signal },
			"Error during signal termination cleanup",
		);
	} finally {
...
export async function handleFatalProcessError(type: string, err: any) {
	...
	try {
		...
		logger.error(
			{ err: shutdownErr?.message || String(shutdownErr) },
			"Error during fatal shutdown cleanup",
		);
	} finally {
...
dictationCoordinator = new DictationControlCoordinator(
	{
		...
		isNativeKeyUpAvailable: () =>
			hotkeyService?.isNativeKeyUpAvailable() ?? false,
...
const pasteCoordinator = new PasteCoordinator((text, isCurrent, beforeWrite) =>
	safePasteService.paste(text, isCurrent, beforeWrite),
);
const selectionClipboardPort = getClipboardPort(
	createElectronClipboardAdapter(addon?.writeClipboardBuffer),
);
...
>(
	{
		...
		loadFile: (win) =>
			win.loadFile(
				fileURLToPath(new URL("../renderer/capture.html", import.meta.url)),
			),
		captureActiveSelection: (timeoutMs, options) =>
			captureActiveSelection(timeoutMs, options),
		capturePasteTarget: () => {
		...
		acknowledgeStart: (seqId, success) =>
			dictationCoordinator.acknowledgeStart(seqId, success),
	},
	recordingLifecycle,
	pasteCoordinator,
);
...
function restoreCapturedSelection(sequenceId?: number) {
	const restored = selectionOwnershipManager.restoreCapturedSelection(
		sequenceId,
		selectionClipboardPort,
	);
	activeSelectionText = "";
...
function cancelDictation(reason: string = "Cancelled") {
	...
	logger.info(
		{ state: currentState, reason },
		"Interrupting/cancelling active dictation flow",
	);
...
export function handleTrustLost() {
	logger.warn(
		"Accessibility trust lost at runtime; cancelling active dictation if any and showing error state",
	);
	if (currentState !== "idle") {
		cancelDictation(
			"Accessibility/Input Monitoring permission revoked at runtime",
		);
	}
	setState(
		"error",
...
function setState(
	state: AppState,
	message?: string,
	options?: { usedPaidKey?: boolean } | boolean,
) {
	currentState = state;
	sequenceId++;
	...
	let usedPaidKey =
		typeof options === "boolean" ? options : Boolean(options?.usedPaidKey);
	if (usedPaidKey) {
	...
	const payload: StatePayload & { hasSelection?: boolean } = {
		...
		hasSelection: Boolean(
			activeSelectionText && activeSelectionText.trim().length > 0,
		),
		usedPaidKey,
	};
	...
	logger.info(
		{
			...
			usedPaidKey: payload.usedPaidKey,
		},
		"State changed",
	);
	...
	if (state === "stopping") {
		stoppingSafetyTimer = setTimeout(() => {
			if (currentState === "stopping") {
				logger.warn(
					"Stopping state timed out, auto-resetting state machine to idle",
				);
      ...
      `;
		...
		exec(`osascript -e '${script}'`, (err) => {
			if (err) {
				...
				logger.info(
					{ activeApp, isTerminal, textLengthToUndo },
					"Executed Voice Undo successfully",
				);
				playUndoChime();
			}
		});
...
function handleVoiceUndoCheck(text: string): boolean {
	...
	const isMatch = exactUndoKeywords.some(
		(kw) => cleaned === kw || (words.length <= 2 && words.includes(kw)),
	);
...
function createHudWindow() {
	...
	hudWindow.loadFile(
		fileURLToPath(new URL("../renderer/hud.html", import.meta.url)),
	);
...
function buildTrayContextMenu(): Menu {
	...
	return Menu.buildFromTemplate([
		...
		},
		{
			...
			click: async () => {
				...
				if (!res.accepted && res.errorCode === "INPUT_MONITORING_REQUIRED") {
					setState(
						"error",
						"Accessibility/Input Monitoring permissions required for Hold Mode",
					);
				}
			},
		},
		{ type: "separator" },
		{
			...
			click: (item) => {
				currentConfig = updateConfig(workingCwd, {
					symbolScannerEnabled: item.checked,
				});
		...
		},
		{
			...
			click: (item) => {
				currentConfig = updateConfig(workingCwd, {
					audioChimesEnabled: item.checked,
				});
			},
		},
		{ type: "separator" },
...
export function enforceIpcSender(
	...
	if (
		result.role === "capture" &&
		!captureRendererSession.isAvailable(event.sender)
	) {
		throw new Error(
			`Unauthorized IPC sender: capture session not available or stale for channel '${channel}'`,
		);
	}
	return result;
}
...
function setupIpcHandlers() {
	ipcMain.on(IPC.RECORDING_DATA, async (event, data: unknown) => {
		...
		if (
			...
			rawByteLength < MIN_STT_PAYLOAD_BYTES
		) {
			sendToCaptureWindow(IPC.CANCEL_RECORDING);
			captureOrchestrator.markCaptureInactive(currentSeq);
			pasteCoordinator.invalidate();
		...
		try {
			...
			if (
				transcriptionSnapshot.sequenceId !== currentSeq ||
				transcriptionSnapshot.state !== "transcribing"
			) {
				logger.warn(
					{ currentSeq, snapshot: transcriptionSnapshot },
			...
			if (usedPaidKey) {
				logger.warn(
					{ modelUsed },
					"STT transcription executed via Paid Fallback Key!",
				);
				try {
			...
			logger.info(
				{ charCount: text.length, usedPaidKey },
			...
			if (!isCurrentTranscription(currentSeq)) {
				logger.warn(
					{ currentSeq },
					"Discarding stale paste attempt after active-app lookup",
				);
				return;
			}
			...
			const cost = calculateDictationCost(
				audioDurationSec,
				text.length,
				modelUsed || currentConfig.geminiModel,
				isEnglish,
			);
			...
			if (!isCurrentTranscription(currentSeq)) {
				logger.warn(
					{ currentSeq },
					"Discarding stale paste attempt before paste coordinator",
				);
				return;
			}
			...
			if (pasteResult.status === "submitted") {
				...
				logger.warn(
					{ pasteResult },
					"Target window changed or paste denied - transcript saved to history",
				);
	...
	ipcMain.on(
		IPC.RECORDING_START_READY,
		(event, payload: { sequenceId: number; deviceStatus?: string }) => {
			void (async () => {
				...
				if (
					...
					seq !== currentSnapshot.sequenceId ||
					(currentSnapshot.state !== "starting" &&
						currentSnapshot.state !== "recording")
				) {
					logger.warn(
						{ seq, currentSnapshot },
						"Received start-ready for invalid sequence or state",
					);
					return;
				}

				const deviceStatus =
					typeof payload?.deviceStatus === "string" &&
					payload.deviceStatus.length > 0 &&
					payload.deviceStatus.length < 256
				...
				if (
					recordingLifecycle.snapshot().sequenceId === seq &&
					currentState === "recording"
				) {
					const finalStatus =
						captureOrchestrator.getSequenceDeviceStatus(seq) || deviceStatus;
					if (finalStatus) {
				...
				logger.error(
					...
					"Error in RECORDING_START_READY handler",
				);
			});
		},
	);
	...
	ipcMain.on(
		IPC.RECORDING_START_FAILED,
		(event, payload: { sequenceId: number; error: string }) => {
	...
	ipcMain.handle(IPC.SAVE_CONFIG, async (event, patch) => {
		...
		const modeChanged =
			validatedPatch.dictationMode &&
			validatedPatch.dictationMode !== previousDictationMode;
		if (
			modeChanged &&
			!["idle", "error"].includes(recordingLifecycle.snapshot().state)
		) {
			throw new Error(
				"Dictation mode can only be changed while recording is idle",
			);
		}
		...
		if (hotkeyService && modeChanged) {
			...
			if (!hotkeyRes.success) {
				currentConfig = updateConfig(workingCwd, {
					dictationMode: previousDictationMode,
				});
				...
				if (!restoreRes.success) {
					logger.error(
						...
						"Failed to restore hotkeys after dictation mode change",
					);
				}
				throw new Error(
					hotkeyRes.error ||
						"Hotkey registration failed after dictation mode change",
				);
			}
		}
		...
		try {
			...
			if (res && res.text !== undefined) {
				...
				return {
					...
					message:
						keys.length > 1
							? `${keys.length} API Keys are valid & saved!`
...
export function handleHotkeyDown(
	...
	if (!isHoldModeInitial && now - lastHotkeyDownTime < 350) {
		logger.warn(
			{ deltaMs: now - lastHotkeyDownTime },
			"Debounced duplicate hotkey down trigger",
		);
		return Promise.resolve({
			accepted: false,
			action: "ignored",
			reason: "Debounced duplicate keydown",
		});
	}
...
export function setDictationCoordinatorForTests(
	coordinator: DictationControlCoordinator,
): void {
	...
	shutdownPromise = (async () => {
		...
		try {
			...
			try {
				...
				logger.warn(
					{ err: err?.message || String(err) },
					"Error aborting active capture flow during shutdown",
				);
			}
			...
			try {
				...
				logger.warn(
					{ err: err?.message || String(err) },
					"Error tearing down capture window during shutdown",
				);
			}
			...
			try {
				...
				logger.warn(
					{ err: err?.message || String(err) },
					"Error restoring selection during shutdown",
				);
			}
			...
			if (hotkeyService) {
				try {
					...
					logger.warn(
						{ err: err?.message || String(err) },
						"Error stopping hotkeys during shutdown",
					);
				}
			}
			...
			try {
				...
				logger.warn(
					{ err: err?.message || String(err) },
					"Error stopping daemon server during shutdown",
				);
			}
		} finally {
			try {
				...
				logger.warn(
					{ err: err?.message || String(err) },
					"Error removing runtime state during shutdown",
				);
			}
		}
...
export async function runStartupSequence(
	...
	try {
		try {
			...
			logger.warn(
				{ err: err?.message || String(err) },
				"Config error during startup, using defaultConfig",
			);
		...
		if (!hotkeyRes.success) {
			logger.warn(
				{ error: hotkeyRes.error },
				"Hotkey registration reported optional degradation on startup",
			);
		}
		...
		logger.info(
			{
				...
				provider: currentConfig.provider,
				geminiModel: currentConfig.geminiModel,
				dictationMode: currentConfig.dictationMode,
			},
			"vo daemon started successfully",
		);
		return true;
	} catch (err: any) {
		logger.error(
			{ err: err?.message || String(err) },
			"Fatal error during application startup; cleaning up and exiting",
		);
		await gracefulShutdown();
		app.exit(1);
		return false;
...
app.whenReady().then(async () => {
	...
	if (process.argv.includes("--headless")) {
		const isOk =
			addon !== null &&
			typeof addon.selfCheck === "function" &&
			addon.selfCheck() === true;
		if (isOk) {
