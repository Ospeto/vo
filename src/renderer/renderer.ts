...
function getSettingsApi(): SettingsElectronAPI | undefined {
	return window.piVoice as SettingsElectronAPI | undefined;
}
...
const translateToggleBtn = document.getElementById(
	"translateToggleBtn",
) as HTMLButtonElement;
...
const editKeycapDisplay = document.getElementById(
	"editKeycapDisplay",
) as HTMLElement;
const recordEditBtn = document.getElementById(
	"recordEditBtn",
) as HTMLButtonElement;
const resetEditShortcutBtn = document.getElementById(
	"resetEditShortcutBtn",
) as HTMLButtonElement;
const editHotkeyFeedback = document.getElementById(
	"editHotkeyFeedback",
) as HTMLElement;

const modalPresetSelect = document.getElementById(
	"modalPresetSelect",
) as HTMLSelectElement;
...
const vocabTagsContainer = document.getElementById(
	"vocabTagsContainer",
) as HTMLElement;

const personNameInput = document.getElementById(
	"personNameInput",
) as HTMLInputElement;
const addPersonNameBtn = document.getElementById(
	"addPersonNameBtn",
) as HTMLButtonElement;
const personNamesContainer = document.getElementById(
	"personNamesContainer",
) as HTMLElement;
const personNamesCountBadge = document.getElementById(
	"personNamesCountBadge",
) as HTMLElement;
const dictionaryPhraseInput = document.getElementById(
	"dictionaryPhraseInput",
) as HTMLInputElement;
const dictionaryAliasesInput = document.getElementById(
	"dictionaryAliasesInput",
) as HTMLInputElement;
const dictionaryCategorySelect = document.getElementById(
	"dictionaryCategorySelect",
) as HTMLSelectElement;
const addDictionaryEntryBtn = document.getElementById(
	"addDictionaryEntryBtn",
) as HTMLButtonElement;
const dictionaryValidation = document.getElementById(
	"dictionaryValidation",
) as HTMLElement;
const dictionaryEntriesContainer = document.getElementById(
	"dictionaryEntriesContainer",
) as HTMLElement;
const vocabTotalCountBadge = document.getElementById(
	"vocabTotalCountBadge",
) as HTMLElement;
...
const geminiApiKey1Input = document.getElementById(
	"geminiApiKey1Input",
) as HTMLInputElement;
const geminiApiKey2Input = document.getElementById(
	"geminiApiKey2Input",
) as HTMLInputElement;
const geminiApiKey3Input = document.getElementById(
	"geminiApiKey3Input",
) as HTMLInputElement;
...
const geminiFallbackApiKeyInput = document.getElementById(
	"geminiFallbackApiKeyInput",
) as HTMLInputElement;
const saveFallbackApiKeyBtn = document.getElementById(
	"saveFallbackApiKeyBtn",
) as HTMLButtonElement;
const fallbackApiKeyFeedback = document.getElementById(
	"fallbackApiKeyFeedback",
) as HTMLElement;

const chimeStartSelect = document.getElementById(
	"chimeStartSelect",
) as HTMLSelectElement;
const chimeEndSelect = document.getElementById(
	"chimeEndSelect",
) as HTMLSelectElement;
const previewStartChimeBtn = document.getElementById(
	"previewStartChimeBtn",
) as HTMLButtonElement;
const previewEndChimeBtn = document.getElementById(
	"previewEndChimeBtn",
) as HTMLButtonElement;
const symbolScannerToggle = document.getElementById(
	"symbolScannerToggle",
) as HTMLInputElement;
const autoEndpointToggle = document.getElementById(
	"autoEndpointToggle",
) as HTMLInputElement;
const transcriptionDelayInput = document.getElementById(
	"transcriptionDelayInput",
) as HTMLInputElement;
const settingModelSelect = document.getElementById(
	"settingModelSelect",
) as HTMLSelectElement;
const micDeviceSelect = document.getElementById(
	"micDeviceSelect",
) as HTMLSelectElement;
const addAppRuleBtn = document.getElementById(
	"addAppRuleBtn",
) as HTMLButtonElement | null;
const newAppNameInput = document.getElementById(
	"newAppNameInput",
) as HTMLInputElement | null;
const newAppPresetSelect = document.getElementById(
	"newAppPresetSelect",
...
function startWaveAnimationLoop() {
	...
	function renderWave() {
		...
		const activeAmp =
			rawRatio < 0.02 ? 0 : Math.min(maxAmp, rawRatio * maxAmp * 1.4);
		...
		if (activeAmp > 0.05) {
			...
			for (let i = 0; i < numLines; i++) {
				...
				for (let x = 0; x <= width; x += 2) {
					...
					const y =
						centerY +
						Math.sin(progress * Math.PI * freqMult + phaseShift) *
							(activeAmp + offsetFactor * 8) *
							envelope;
				...
				spectrumCtx.lineWidth =
					i === 2 || i === 3 ? Math.min(3.2, 1.8 + rawRatio * 2) : 1.2;
...
export async function initUI() {
	...
	if (config) {
		...
		if (presetSelect && config.dictationPreset)
			presetSelect.value = config.dictationPreset;
		if (modeSelect && config.dictationMode)
			modeSelect.value = config.dictationMode;
		...
		dictionaryEntries = Array.isArray(config.dictionaryEntries)
			? config.dictionaryEntries.map((entry: DictionaryEntry) => ({
					...entry,
					spokenAliases: [...entry.spokenAliases],
				}))
			: [];
		if (config.presetVocabulary) {
			currentPresetVocabMap = config.presetVocabulary as Record<
				string,
				string[]
			>;
		}
	...
	if (micDeviceSelect) {
		micDeviceSelect.addEventListener("change", async () => {
			await getSettingsApi()?.saveConfig({
				audioDeviceId: micDeviceSelect.value,
			});
		});
	}
	...
	if (addAppRuleBtn) {
		addAppRuleBtn.addEventListener("click", async () => {
			...
			if (name && preset) {
				...
				renderAppRules();
				await getSettingsApi()?.saveConfig({
					appPresetMappings: currentAppMappings,
				});
			}
		});
	}
...
function renderDictionaryEntries(highlightId?: string) {
	...
	const filteredEntries = dictionaryEntries.filter((entry) => {
		...
		if (currentVocabFilter === "person_name")
			return entry.category === "person_name";
		if (currentVocabFilter === "technical")
	...
	for (const entry of filteredEntries) {
		...
		toggle.addEventListener("click", () =>
			updateDictionaryEntry({ ...entry, enabled: !entry.enabled }),
		);
		...
		edit.addEventListener("click", () => {
			...
			if (dictionaryCategorySelect)
				dictionaryCategorySelect.value = entry.category || "general";
		...
		remove.addEventListener("click", () =>
			saveDictionaryEntries(
				dictionaryEntries.filter((item) => item.id !== entry.id),
			),
		);
...
async function saveDictionaryEntries(
	nextEntries: DictionaryEntry[],
	highlightId?: string,
) {
	const errors = validateDictionaryEntries(nextEntries);
	if (errors.length > 0) {
		showDictionaryError(
			errors.map((error) => `${error.alias}: ${error.message}`).join("; "),
		);
		return false;
	}
	try {
		const config = await getSettingsApi()?.saveConfig({
			dictionaryEntries: nextEntries,
		});
		dictionaryEntries = (config?.dictionaryEntries ||
			nextEntries) as DictionaryEntry[];
		...
		showDictionaryError(
			error instanceof Error
				? error.message
				: "Could not save dictionary entry",
		);
		return false;
	}
}
...
async function updateDictionaryEntry(entry: DictionaryEntry) {
	await saveDictionaryEntries(
		dictionaryEntries.map((item) => (item.id === entry.id ? entry : item)),
		entry.id,
	);
}
...
async function saveDictionaryEntry() {
	...
	const aliases = (dictionaryAliasesInput?.value || "")
		.split(",")
		.map((alias) => alias.trim())
		.filter(Boolean);
	const category =
		(dictionaryCategorySelect?.value as VocabularyCategory) || "general";
	...
	const existingEntry = !editingDictionaryId
		? dictionaryEntries.find(
				(entry) =>
					entry.phrase.trim().normalize("NFKC").toLocaleLowerCase() ===
					normPhrase,
			)
		: null;

	const targetId =
		editingDictionaryId || existingEntry?.id || crypto.randomUUID();
	...
	const isExistingInList = dictionaryEntries.some(
		(entry) => entry.id === targetId,
	);
	const nextEntries = isExistingInList
		? dictionaryEntries.map((entry) =>
				entry.id === targetId ? nextEntry : entry,
			)
	...
	if (success) {
		showDictionaryMessage(
			`✓ Saved "${phrase} ← ${mergedAliases.join(", ")}"`,
			false,
		);
		...
		if (addDictionaryEntryBtn)
			addDictionaryEntryBtn.textContent = "+ Add Entry";
	}
}
...
export function renderPersonNames() {
	const container =
		personNamesContainer || document.getElementById("personNamesContainer");
	if (!container) return;
	container.textContent = "";

	const badge =
		personNamesCountBadge || document.getElementById("personNamesCountBadge");
	if (badge) {
...
export function renderVocabTags() {
	const container =
		vocabTagsContainer || document.getElementById("vocabTagsContainer");
	const presetSelect =
		modalPresetSelect ||
		(document.getElementById("modalPresetSelect") as HTMLSelectElement);
...
export async function addVocabTerm(preset: string, term: string) {
	...
	if (!list.includes(cleanTerm)) {
		...
		await getSettingsApi()?.saveConfig({
			presetVocabulary: currentPresetVocabMap,
		});
		renderVocabTags();
	}
...
export async function renderHistory() {
	...
	const history: HistoryEntry[] = (await getSettingsApi()?.getHistory()) || [];
...
let settingsUnsubscribes: Array<() => void> = [];

const api = window.piVoice as SettingsElectronAPI | undefined;

if (api?.onStateChanged) {
	const unsub = api.onStateChanged((payload: StatePayload) => {
		if (payload.state === "recording") {
			...
			if (payload.state === "idle") {
				if (
					payload.message?.startsWith("Dictated:") ||
					payload.message?.includes("Key:")
				) {
					playBassyEndChime();
				}
				renderHistory();
	...
	if (unsub) settingsUnsubscribes.push(unsub);
}

if (api?.onAudioLevelUpdate) {
	const unsub = api.onAudioLevelUpdate(
		(payload: AudioLevelPayload | number) => {
...
window.addEventListener("beforeunload", cleanupSettingsSubscriptions);
window.addEventListener("unload", cleanupSettingsSubscriptions);
...
export function enableControls(enabled: boolean) {
	...
	document
		.querySelectorAll<HTMLButtonElement>(".preset-pill-btn")
		.forEach((btn) => {
			btn.disabled = !enabled;
		});
	document
		.querySelectorAll<HTMLButtonElement>(".model-pill-btn")
		.forEach((btn) => {
			btn.disabled = !enabled;
		});
...
document.querySelectorAll(".preset-pill-btn").forEach((btn) => {
	btn.addEventListener("click", async () => {
		...
		await getSettingsApi()?.saveConfig({ dictationPreset: selectedPreset });
	});
});
...
document.querySelectorAll(".model-pill-btn").forEach((btn) => {
	btn.addEventListener("click", async () => {
		...
		await getSettingsApi()?.saveConfig({
			geminiModel: selectedModel as GeminiModelChoice,
		});
	});
});
...
modeSelect?.addEventListener("change", async () => {
	...
	await getSettingsApi()?.saveConfig({ dictationMode: selectedMode });
});
...
gainSlider?.addEventListener("change", async () => {
	const val = parseFloat(gainSlider.value);
	await getSettingsApi()?.saveConfig({ inputGain: val });
});

modelSelect?.addEventListener("change", async () => {
	const selectedModel = modelSelect.value as GeminiModelChoice;
	if (settingModelSelect) settingModelSelect.value = selectedModel;
	await getSettingsApi()?.saveConfig({ geminiModel: selectedModel });
});
...
clearHistoryBtn?.addEventListener("click", async () => {
	await getSettingsApi()?.clearHistory();
	await renderHistory();
});
...
dictionaryAliasesInput?.addEventListener("input", () =>
	showDictionaryError(""),
);
...
document.querySelectorAll(".vocab-filter-btn").forEach((btn) => {
	btn.addEventListener("click", () => {
		document
			.querySelectorAll(".vocab-filter-btn")
			.forEach((b) => b.classList.remove("active"));
		btn.classList.add("active");
...
const testApiKeyBtn = document.getElementById(
	"testApiKeyBtn",
) as HTMLButtonElement;
...
testApiKeyBtn?.addEventListener("click", async () => {
	...
	const res = await (
		window.piVoice as SettingsElectronAPI | undefined
	)?.testApiKey(keyVal);
	if (apiKeyFeedback) {
		if (res?.success) {
...
saveApiKeyBtn?.addEventListener("click", async () => {
	...
	if (apiKeyFeedback) {
		...
		apiKeyFeedback.textContent = updatedConfig?.hasGeminiKey
			? `✓ Saved ${keyCount || 1} Primary Gemini Key(s)!`
			: "Primary API Keys cleared.";
		apiKeyFeedback.style.color = updatedConfig?.hasGeminiKey
			? "#10b981"
			: "#ef4444";
		apiKeyFeedback.style.display = "block";
		setTimeout(() => {
...
saveFallbackApiKeyBtn?.addEventListener("click", async () => {
	const fallbackKeyVal = geminiFallbackApiKeyInput?.value
		? geminiFallbackApiKeyInput.value.trim()
		: "";
	const updatedConfig = await getSettingsApi()?.saveConfig({
		geminiFallbackApiKey: fallbackKeyVal,
	});
	if (geminiFallbackApiKeyInput) {
		geminiFallbackApiKeyInput.value = "";
	...
	if (fallbackApiKeyFeedback) {
		fallbackApiKeyFeedback.textContent = updatedConfig?.hasGeminiFallbackKey
			? "✓ Paid Fallback API Key saved!"
			: "Fallback API Key cleared.";
		fallbackApiKeyFeedback.style.color = updatedConfig?.hasGeminiFallbackKey
			? "#10b981"
			: "#a1a1aa";
...
chimeStartSelect?.addEventListener("change", async () => {
	await getSettingsApi()?.saveConfig({
		chimeSoundStart: chimeStartSelect.value as ChimeSoundChoice,
	});
});

chimeEndSelect?.addEventListener("change", async () => {
	await getSettingsApi()?.saveConfig({
		chimeSoundEnd: chimeEndSelect.value as ChimeSoundChoice,
	});
});

symbolScannerToggle?.addEventListener("change", async () => {
	await getSettingsApi()?.saveConfig({
		symbolScannerEnabled: symbolScannerToggle.checked,
	});
});

autoEndpointToggle?.addEventListener("change", async () => {
	await getSettingsApi()?.saveConfig({
		autoEndpointEnabled: autoEndpointToggle.checked,
	});
});
...
transcriptionDelayInput?.addEventListener("change", async () => {
	...
	if (!isNaN(val)) {
		...
		await getSettingsApi()?.saveConfig({ transcriptionDelaySec: clamped });
	}
});

previewStartChimeBtn?.addEventListener("click", async () => {
	await getSettingsApi()?.previewChime(chimeStartSelect.value);
});

previewEndChimeBtn?.addEventListener("click", async () => {
	await getSettingsApi()?.previewChime(chimeEndSelect.value);
});

resetShortcutBtn?.addEventListener("click", async () => {
	const res = await getSettingsApi()?.registerHotkey("ctrl+cmd+option+v");
	if (res?.success) {
...
function startHotkeyRecording() {
	...
	const handleKeyDown = async (e: KeyboardEvent) => {
		...
		if (!isModifierKey) {
			...
			const res = await getSettingsApi()?.registerHotkey(keyStr);
			if (res?.success) {
...
resetEditShortcutBtn?.addEventListener("click", async () => {
	const res = await getSettingsApi()?.registerEditHotkey("ctrl+cmd+option+e");
	if (res?.success) {
...
function startEditHotkeyRecording() {
	...
	const handleKeyDown = async (e: KeyboardEvent) => {
		...
		if (!isModifierKey) {
			...
			const keyStr = [...modifiers, mainKey].join("+");
			const res = await getSettingsApi()?.registerEditHotkey(keyStr);
			if (res?.success) {
...
window.addEventListener("keydown", (e) => {
	if (e.key === "Escape" && !isRecordingHotkey && !isRecordingEditHotkey) {
		getSettingsApi()?.cancelDictation?.();
	}
});
