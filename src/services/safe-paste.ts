...
export type SafePasteFailureReason =
	| TargetFailureReason
	| InjectionFailureReason
	| ClipboardFailureReason;
export type SafePasteResult =
	| { ok: true; reason: "injection_requested" }
	| { ok: false; reason: SafePasteFailureReason };
export type TargetReadResult = {
	target: TargetIdentity | null;
	reason: TargetFailureReason | null;
};
export type SafePasteDiagnosticStage =
	| "target_capture"
	| "target_recheck"
	| "clipboard_snapshot"
	| "clipboard_write"
	...
	| "clipboard_restore"
	| "total";
export type SafePasteDiagnosticOutcome =
	| "captured"
	| "failed"
	| "not_run"
...
export type SafePasteDiagnostics = {
	emit: (diagnostic: SafePasteDiagnostic) => void;
	now?: () => number;
};
...
export function parseTargetLine(line: string): TargetIdentity | null {
	...
	return /^\d+$/.test(pidText) &&
		/^\d+$/.test(windowIdText) &&
		isValidTarget(parsed)
		? parsed
		: null;
}
...
export function sameTarget(a: TargetIdentity, b: TargetIdentity): boolean {
	return (
		isValidTarget(a) &&
		isValidTarget(b) &&
		normalize(a.bundleId) === normalize(b.bundleId) &&
		normalize(a.appName) === normalize(b.appName) &&
		a.pid === b.pid
	);
}

const targetResult = (
	value: TargetIdentity | null | TargetReadResult,
): TargetReadResult => {
	if (value && typeof value === "object" && "target" in value) return value;
	return value
		? { target: value, reason: null }
		: { target: null, reason: "target_unavailable" };
};
const isSafeFailureReason = (value: unknown): value is SafePasteFailureReason =>
	value === "target_unavailable" ||
	value === "target_malformed" ||
	value === "target_mismatch" ||
	value === "native_unavailable" ||
	value === "permission_blocked" ||
	value === "injection_rejected" ||
	value === "injection_failed" ||
	value === "clipboard_snapshot_failed" ||
	value === "clipboard_write_failed" ||
	value === "clipboard_restore_failed";
const safeReason = (
	value: unknown,
	fallback: SafePasteFailureReason,
): SafePasteFailureReason => (isSafeFailureReason(value) ? value : fallback);
const failureReason = (
	error: unknown,
	fallback: SafePasteFailureReason,
): SafePasteFailureReason => {
	const reason =
...
const sameClipboardSnapshot = (
	...
	if (a.image && b.image && a.image !== b.image) {
		const dataA =
			typeof (a.image as any).toDataURL === "function"
				? (a.image as any).toDataURL()
				: null;
		const dataB =
			typeof (b.image as any).toDataURL === "function"
				? (b.image as any).toDataURL()
				: null;
		if (dataA !== dataB) return false;
	}
	return a.formats.every((item) => {
		const other = b.formats.find(
			(candidate) =>
				candidate.format.toLowerCase() === item.format.toLowerCase(),
		);
...
export class SafePasteService {
	...
	constructor(
		private readonly readTarget: () =>
			...
			| null
			| TargetReadResult
			| Promise<TargetIdentity | null | TargetReadResult>,
		private readonly injectPaste: (
			expected: TargetIdentity,
			isCurrent: () => boolean,
		) => Promise<void>,
		private readonly clipboard: ClipboardPort,
		private readonly authorizeTarget: (
			expected: TargetIdentity,
		) => Promise<void> = async (expected) => {
			const current = targetResult(await this.readTarget());
			if (!current.target)
				throw Object.assign(new Error(current.reason ?? "target_unavailable"), {
					reason: current.reason ?? "target_unavailable",
				});
			if (!isValidTarget(current.target))
				throw Object.assign(new Error("target_malformed"), {
					reason: "target_malformed",
				});
			if (!sameTarget(expected, current.target))
				throw Object.assign(new Error("target_mismatch"), {
					reason: "target_mismatch",
				});
		},
		private readonly diagnostics?: SafePasteDiagnostics,
		private readonly hold: SafePasteHold = (durationMs) =>
	...
	captureTarget(): TargetIdentity | null {
		...
		if (capturedValue instanceof Promise) {
			this.capturePromise = capturedValue.then((value) => {
				this.applyCapture(targetResult(value));
	...
	private applyCapture(captured: TargetReadResult): void {
		this.capturedTarget =
			captured.target && isValidTarget(captured.target)
				? captured.target
				: null;
		this.captureFailure =
			captured.target && !isValidTarget(captured.target)
	...
	async paste(
		...
		const captureReason =
			this.captureFailure ??
			(this.capturedTarget ? null : "target_unavailable");
		this.emit({
			operationId,
			stage: "target_capture",
			durationMs: this.captureDuration,
			outcome: this.capturedTarget ? "captured" : "failed",
		...
		if (!isValidTarget(expectedTarget)) {
			this.emit({
				...
				durationMs: 0,
				outcome: "not_run",
				reason: captureReason ?? "target_unavailable",
			});
			const result = {
				ok: false as const,
				reason: captureReason ?? ("target_unavailable" as TargetFailureReason),
			};
		...
		try {
			...
			this.emit({
				operationId,
				stage: "target_recheck",
				durationMs: this.elapsed(recheckStarted),
				outcome: "authorization_accepted",
			});
		} catch (error) {
		...
		try {
			...
			this.emit({
				...
				durationMs: this.elapsed(snapshotStarted),
				outcome: "success",
			});
			if (
				previous.formats.length > 0 &&
				!this.clipboard.preservesCustomFormats
			) {
				const result = {
					ok: false as const,
					reason: "clipboard_restore_failed" as ClipboardFailureReason,
				};
				this.emit({
					operationId,
			...
			const result = {
				ok: false as const,
				reason: "clipboard_snapshot_failed" as ClipboardFailureReason,
			};
			this.emit({
				operationId,
				stage: "clipboard_snapshot",
		...
		try {
			...
			this.emit({
				...
				durationMs: this.elapsed(writeStarted),
				outcome: "success",
			});
		} catch (error) {
			...
			if (!isCurrent()) {
				...
				try {
					...
					this.emit({
						operationId,
						stage: "injection",
						durationMs: this.elapsed(injectionStarted),
						outcome: "success",
					});
		...
		if (result?.ok) {
			...
			try {
				...
				this.emit({
					operationId,
					stage: "clipboard_hold",
					durationMs: this.elapsed(holdStarted),
					outcome: "success",
				});
			} catch {
				this.emit({
		...
		try {
			...
			if (writtenSnapshot && sameClipboardSnapshot(current, writtenSnapshot)) {
				...
				this.emit({
					operationId,
					stage: "clipboard_restore",
					durationMs: this.elapsed(restoreStarted),
					outcome: "success",
				});
			} else {
	...
	private clock(): number {
		try {
			...
			if (sampled === undefined || Number.isFinite(sampled))
				return sampled ?? this.monotonicClock();
		} catch {
	...
	private emit(
		diagnostic: Omit<SafePasteDiagnostic, "targetMode"> & {
			targetMode?: "window";
		},
	): void {
		try {
			this.diagnostics?.emit({ ...diagnostic, targetMode: "window" });
		} catch {
			/* diagnostics must not affect paste */
		}
	}
	private emitTotal(
		operationId: string,
		started: number,
		result: SafePasteResult,
	): void {
		this.emit({
			operationId,
			stage: "total",
...
type ClipboardWriteData<TImage extends ClipboardImage> = {
	text?: string;
	html?: string;
	rtf?: string;
	image?: TImage;
...
export type ClipboardPort<TImage extends ClipboardImage = ClipboardImage> = {
	readText(): string;
	writeText(text: string): void;
	snapshot(): ClipboardSnapshot & { image?: TImage };
	restore(snapshot: ClipboardSnapshot & { image?: TImage }): void;
	preservesCustomFormats?: boolean;
};
...
export function createClipboardPort<TImage extends ClipboardImage>(
	clipboard: ClipboardAdapter<TImage>,
): ClipboardPort<TImage> {
	const standardFormats = new Set([
		"text/plain",
		"text/html",
		"text/rtf",
		"image/png",
	...
	const hasFormat = (formats: string[], name: string): boolean =>
		formats.some((format) => format.toLowerCase() === name);
	const hasImage = (image: TImage | undefined): boolean =>
		image !== undefined && !(image.isEmpty?.() ?? false);
	return {
		preservesCustomFormats: Boolean(
			clipboard.writeBufferIsAdditive && clipboard.writeBuffer,
		),
		...
		snapshot: () => {
			...
			return {
				text:
					text &&
					(hasFormat(formats, "text/plain") || !clipboard.availableFormats)
						? text
						: undefined,
				...
				image:
					hasImage(image) &&
					formats.some((format) => format.toLowerCase().startsWith("image/"))
						? image
						: undefined,
				formats: formats
					.filter((format) => !standardFormats.has(format.toLowerCase()))
					.map((format) => ({
						format,
						data: clipboard.readBuffer?.(format) ?? Buffer.alloc(0),
					})),
			};
		},
...
export function createMacSafePasteService<TImage extends ClipboardImage>(
	addon: NativePasteAddon | null,
	clipboard: ClipboardAdapter<TImage>,
): SafePasteService {
	const readTarget = (): TargetReadResult => {
		...
		try {
			...
			return result.ok
				? { target: result, reason: null }
				: {
						target: null,
						reason:
							result.reason === "permission_blocked"
								? "target_unavailable"
								: (result.reason as TargetFailureReason),
					};
		} catch {
			return { target: null, reason: "native_unavailable" };
		}
	};
	const authorizeTarget = async (expected: TargetIdentity): Promise<void> => {
		if (!addon)
			throw Object.assign(new Error("native addon unavailable"), {
				reason: "native_unavailable",
			});
		try {
			const result = addon.authorize(expected);
			if (!result.ok)
				throw Object.assign(new Error(result.reason), {
					reason: result.reason,
				});
		} catch (error) {
			throw Object.assign(new Error("native authorization failed"), {
				reason: failureReason(error, "target_unavailable"),
			});
		}
	};
	const injectPaste = async (
		expected: TargetIdentity,
		isCurrent: () => boolean,
	): Promise<void> => {
		if (!addon)
		...
		if (!isCurrent())
			throw Object.assign(new Error("paste invalidated"), {
				reason: "target_mismatch",
			});
		try {
			const result = addon.inject(expected);
			if (!result.ok)
				throw Object.assign(new Error(result.reason), {
					reason: result.reason,
				});
		} catch (error) {
			throw Object.assign(new Error("native injection failed"), {
				reason: failureReason(error, "injection_rejected"),
			});
		}
	};
	return new SafePasteService(
		readTarget,
		injectPaste,
