...
export function isTechnicalPath(cleanPath: string): boolean {
	...
	if (
		extMatch &&
		extMatch[1] &&
		TECHNICAL_EXTENSIONS.has(extMatch[1].toLowerCase())
	) {
		return true;
	}
...
export function extractTechnicalTokens(text: string): string[] {
	...
	const isSingleCapitalizedWord = (s: string): boolean =>
		/^[A-Z][a-z]+$/.test(s);
	...
	const pascalCaseMatches = text.match(
	...
	const cliCmdMatches = text.match(
		/\b(?:bun|npm|pnpm|yarn|npx|cargo|git|docker|pip|python|node|deno)\s+[a-z0-9_:-]+\b/gi,
	);
	if (cliCmdMatches) {
	...
	try {
		...
		if (options.symbolScannerEnabled !== false) {
			...
			try {
				...
				logger.debug(
					{ err: String(scanErr) },
					"Failed to scan workspace symbols for two-step translation",
				);
			}
		}
