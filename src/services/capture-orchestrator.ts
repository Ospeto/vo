...
import type { RendererSession } from "./renderer-session.js";
...
export interface CaptureOrchestratorOptions<TWindow = any, TSender = any> {
	...
	onRenderProcessGone: (
		sender: TSender,
		handler: (event: any, details: any) => void,
	) => void;
...
export class CaptureOrchestrator<TWindow = any, TSender = any> {
	...
	constructor(
		...
		this.pasteCoordinator =
			pasteCoordinator ??
			new PasteCoordinator(async () => ({
				ok: true,
	...
	public restoreCapturedSelection(sequenceId?: number): boolean {
		try {
			const restored = selectionOwnershipManager.restoreCapturedSelection(
				sequenceId,
				this.options.selectionClipboardPort,
			);
			...
			logger.warn(
				...
				"Failed to restore captured selection",
			);
			this.activeSelectionText = "";
			return false;
		}
	...
	public markCaptureInactive(sequenceId?: number): void {
		...
		if (
			...
			sequenceId === this.activeCaptureSequenceId
		) {
	...
	public isCaptureActive(newSequenceId?: number): boolean {
		...
		if (
			this.activeCaptureSequenceId !== null &&
			this.activeCaptureSequenceId !== newSequenceId
		) {
			return true;
		}
		return false;
	...
	public enforceIpcSender(
		...
		const popoverWin = this.options.getPopoverWindow
			? this.options.getPopoverWindow()
			: null;
		const hudWin = this.options.getHudWindow
			? this.options.getHudWindow()
			: null;
	...
	public async startRecordingFlow(): Promise<boolean> {
		...
		if (this.isCaptureActive(reqRes.sequenceId)) {
			logger.warn(
				"Cannot start recording: prior capture is active or tearing down",
			);
			this.pasteCoordinator.invalidate();
		...
		const targetSender = this.options.getWebContents(
		...
		const sent = this.sendToCaptureWindow(
			IPC.START_RECORDING,
			"webm",
			inputGain,
			reqRes.sequenceId,
		...
		if (
			...
			!["starting", "recording"].includes(lifecycleSnapshot.state)
		) {
			if (
				lifecycleSnapshot.sequenceId === reqRes.sequenceId &&
				this.controller.getCaptureWindow() &&
		...
		logger.info(
			{
				triggerMode: this.currentTriggerMode,
				hasSelection: selection.hasSelection,
				selectionLength: this.activeSelectionText.length,
			},
		...
		} else if (this.lifecycle.snapshot().state === "starting") {
