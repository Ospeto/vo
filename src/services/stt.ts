	...
	try {
		...
		if (term.length > 0) {
			...
			logger.info(
				...
				"Appended new term to personal vocabulary dictionary",
			);
		}
	} catch (err) {
		logger.error(
			{ err: String(err) },
			"Failed to append term to personal vocabulary dictionary",
		);
	}
}
...
export function prewarmGeminiClient(): void {
	try {
		...
		logger.warn(
			{ err: String(err) },
			"Failed to pre-warm Gemini client connection",
		);
	}
}

import {
	loadNativePasteAddon,
	resolveNativePastePath,
} from "./native-paste-addon.js";
...
export async function getActiveAppName(): Promise<string> {
	...
	try {
		...
		logger.debug(
			{ err: String(err) },
			"Failed to capture active app via native addon",
		);
	}
...
export function getPresetPromptInstructions(
	preset?: DictationPreset,
...
Translate the developer's spoken Burmese/English dictation directly into clean, precise, professional English technical specifications optimized for AI coding assistants (Pi / Cursor / Antigravity / Claude Code / Copilot).
...
3. CONCISE TECHNICAL IMPERATIVES & FILLER STRIPPING: Convert spoken thoughts into direct software engineering imperatives (e.g., "user id မပါရင် ဘာမှမလုပ်ဘဲ ပြန်ထွက်" -> "Return early if userId is null or undefined"). Strip conversational filler ("uh", "I think", "maybe like", "let's see", "you know") while keeping technical intent exact.
4. FAITHFUL TRANSLATION & ZERO IMPROVISATION: Translate spoken Burmese/English directly into clean technical English specifications. Do NOT invent unmentioned requirements, unsaid state management, unsaid code blocks, or extra architectural steps. Output ONLY what the user explicitly dictated.
5. SPOKEN IDENTIFIER FORMATTING: Convert spoken variable naming cues into precise code identifiers as secondary formatting:
...
6. STRICT ENGLISH ONLY (ZERO BURMESE SCRIPT): Output ONLY pure English text. Under NO circumstances should any Burmese script, Burmese characters (မြန်မာစာ), conversational preambles ("Here is the specification:"), or raw dictation repeats be included.
7. ZERO BOILERPLATE & ZERO PREAMBLES: Output NO conversational intros, commentary, or unrequested code blocks. Return ONLY clean technical specifications ready for AI coding agents.
`.trim();
			}
			return `
Preset Mode: SYSTEMATIC CODE DICTATION & TECHNICAL INSTRUCTION (DETECT MODE).
Transcribe spoken audio (Burmese or English) faithfully in its original spoken language. Format dictation into clean, structured technical commands or code comments optimized for AI coding assistants (Pi / Cursor / Antigravity / Claude Code / Copilot). Do NOT force English translation.
...
2. AI AGENT PROMPT STRUCTURING & INTENT MAPPING: Format informal spoken dictation into structured technical directives or code comments (# or //). Map spoken engineering intents ("refactor", "fix bug", "add test", "clean up", "why is this failing") into clean directives while stripping spoken filler ("uh", "I think", "maybe like", "let's see").
3. SYNTAX-FRIENDLY CODE FORMATTING & COMMENTS: Apply syntax-friendly code formatting, inline backticks for code symbols (\`userId\`, \`created_at\`), and inline code comment structure (# or //) where dictated.
4. SPOKEN IDENTIFIER FORMATTING: Convert spoken variable naming cues into precise code identifiers as secondary formatting:
   - "camel case user id" -> userId
   - "snake case created at" -> created_at
   - "pascal case user response" -> UserResponse
   - "upper case api key" -> API_KEY
   - "kebab case user-card" -> user-card
5. ZERO CONVERSATIONAL PREAMBLES & ZERO BOILERPLATE: Output NO conversational intros (e.g. "Here is the specification:"), NO unrequested boilerplate code generation, and NO arbitrary rewriting. Output ONLY clean, direct prompts/specifications or inline code comments ready for AI coding agents.
`.trim();
		case "translate":
			return `
...
const SAFE_REPEAT_WORD_REGEX = new RegExp(
	`\\b(${SAFE_ENGLISH_REPEAT_WORDS})[ \\t]+\\1\\b`,
	"gi",
);
const CODE_REGION_REGEX = /(```[\s\S]*?```|`[^`\n]*`)/g;

function transformOutsideCodeRegions(
	text: string,
...
export const BURMESE_UNICODE_REGEX =
	/[\u1000-\u109F\uAA60-\uAA7F\uA9E0-\uA9FF]/;
...
export function sanitizeCodePresetText(text: string): string {
	...
	return transformOutsideCodeRegions(text.trim(), (segment) => {
		...
		cleaned = cleaned.replace(
			/^(Here is the (?:specification|spec|code comment|comment):)\s*/gi,
			"",
		);
...
export function sanitizeTranscribedText(
	text: string,
	activeApp?: string,
	preset?: DictationPreset,
	...
	const effectiveDictionaryEntries =
		dictionaryEntries ?? loadPersistedVocabulary().entries ?? [];
	...
	cleaned = cleaned.replace(
		/\s*(စာကြောင်းသစ်|\bnew line\b|\bnewline\b)\s*/gi,
		"\n",
	);

	// 3. Convert spoken bullet point commands
	cleaned = cleaned.replace(
		/(^|\n)\s*(\bbullet point\b|\bbullet\b|အချက်)\s+/gi,
		"$1- ",
	);
	...
	cleaned = cleaned.replace(
		/\s*(အာမေဋိတ်|\bexclamation mark\b|\bexclamation point\b)\s*/gi,
		"! ",
	);
	...
	cleaned = cleaned.replace(
		/(?:^|\s+)(?:အာ+|ဟာ+|အင်း+|အင်+|အမ်+|အမ်း+|အာ့+)(?:[။.,…\-–—\s]*)(?=\s|[။.,…]|$)/g,
		" ",
	);
	cleaned = cleaned.replace(
		/^(ဟိုဟာလေ|ဟိုဟာ|အာ့ဆို|အာ့|အင်းး|အင်း|အမ်မ်|အမ်|ဒီဥစ္စာ|အာ)\s*,?\s*/gi,
		"",
	);
	...
	cleaned = transformOutsideCodeRegions(cleaned, (segment) =>
		segment
			.replace(/[ \t]+/g, " ")
			.replace(/။+/g, "။")
			.replace(/၊+/g, "၊")
...
function migrateLegacyHintEntries(terms: string[]): DictionaryEntry[] {
	return terms
		.map((term) => dictionaryEntryFromTerm(term))
...
export function prepareHintEntries(
	...
	if (!entries || entries.length === 0 || entryLimit === 0 || termLimit === 0)
		return [];
	...
	for (const entry of enabled) {
		...
		const rawAliases = Array.isArray(entry.spokenAliases)
			? entry.spokenAliases
			: [];
		...
		if (seenPhrases.has(normPhrase)) {
			...
			if (existing) {
				for (const alias of validAliases) {
					const normAlias = alias.normalize("NFKC").toLocaleLowerCase();
					if (
						!existing.spokenAliases.some(
							(a) => a.normalize("NFKC").toLocaleLowerCase() === normAlias,
						)
					) {
			...
			prepared.push({
				id: entry.id,
				phrase,
				spokenAliases: boundedAliases.filter(
					(a) => a.normalize("NFKC").toLocaleLowerCase() !== normPhrase,
				),
				enabled: true,
...
export function buildOpenAIVocabularyPrompt(
	entries: DictionaryEntry[],
): string {
...
async function transcribeGemini(
	...
	const resolvedPreset = resolveEffectivePreset(
		dictationPreset,
		activeApp,
		appMappings,
	);
	...
	let userPromptText =
		"Transcribe the spoken audio accurately in its original spoken language (Burmese or English).";
	...
	if (hasSelectedText) {
		...
		const codeDirective = isCodePreset
			? `\nCODING PRESET DIRECTIVE: Coding mode is ACTIVE. Convert informal spoken Burmese/English dictation into clean, direct technical specifications and imperatives optimized for AI coding assistants (Pi, Cursor, Antigravity, Claude Code). Apply syntax-friendly code formatting, inline backticks, or inline code comments as requested. Convert spoken variable naming cues as secondary formatting. Ensure ZERO conversational preambles ("Here is the specification:") and ZERO unrequested code block boilerplate.`
			: "";
...
OUTPUT FORMAT: Return ONLY the final result text without any quotes, introductory phrases, or explanatory commentary.`;
		const formattedSelectedText = formatSelectedTextForPrompt(
			selectedText!.trim(),
		);
		...
		if (isTranslationActive) {
			sttBasePrompt = `You are an expert real-time Speech Translator and Code Specification Architect. Your single imperative task is to listen to the spoken audio and output ONLY its clean, technical translation into ${resolvedTargetLang} with software engineering specification formatting. Convert informal spoken Burmese/English dictation into clean, direct technical specifications and imperatives (Goal, Target, Action, Constraints) optimized for AI coding assistants (Pi, Cursor, Antigravity, Claude Code). Ensure ZERO conversational preambles ("Here is the specification:"), ZERO unrequested boilerplate code generation, and ZERO arbitrary rewriting. Output ONLY clean, direct prompts/specifications or inline code comments ready for AI coding assistants (Pi, Cursor, Antigravity, Claude Code).`;
			...
			sttBasePrompt = `You are an expert real-time Speech Dictation and Code Specification Architect. Your single imperative task is to listen to the spoken audio (in Burmese, English, or technical code terms) and transcribe/format it accurately in its original spoken language with syntax-friendly formatting, technical directives, inline backticks, and inline code comment structure. Format dictation into clean, structured technical commands or code comments optimized for AI coding assistants (Pi, Cursor, Antigravity, Claude Code). Preserve the spoken language (Burmese or English) faithfully. Ensure ZERO conversational preambles ("Here is the specification:"), ZERO unrequested boilerplate code generation, and ZERO arbitrary rewriting. Output ONLY clean, direct prompts/specifications or inline code comments ready for AI coding assistants.`;
			userPromptText =
				"Transcribe the spoken audio with syntax-friendly technical code formatting in its original spoken language. Do NOT force translation unless auto-translation is active.";
		}
	...
	const dynamicTimeoutMs = Math.max(
		12000,
		Math.min(25000, Math.ceil(audioBuffer.length / 5)),
	);
	...
	try {
		...
		logger.debug(
			{ err: String(err) },
			"Failed to check paid config for Gemini STT",
		);
	}
		...
		try {
			...
			if (fallbackClient && !paidFallbackInFlight && !paidFallbackAttempted) {
				...
				const primaryDelayTimer = new Promise<never>((_, reject) => {
					timerId = setTimeout(
						() => reject(new Error("Primary Key delay > 2000ms")),
						2000,
					);
				...
				resultPromise = (async () => {
					try {
						...
						if (timerId) clearTimeout(timerId);
						logger.info(
							{ model, err: primaryErr?.message },
							"Primary API Key took >2000ms or errored; triggering Parallel Paid Fallback Key",
						);
						try {
			...
			const timeoutPromise = new Promise<never>((_, reject) => {
				...
				if (
					timeoutTimerId &&
					typeof timeoutTimerId === "object" &&
					"unref" in timeoutTimerId
				)
					(timeoutTimerId as any).unref();
			});
			...
			if (
				...
				if (!abortSignal?.aborted) {
					logger.info(
						{ model, charCount: text.length },
						"Gemini STT output contained Burmese script in English preset, executing text translation fallback",
					);
					try {
						...
						if (
							translatedText &&
							!/[\u1000-\u109F\uAA60-\uAA7F\uA9E0-\uA9FF]/.test(translatedText)
						) {
							text = translatedText;
						}
					} catch (translateErr: any) {
						logger.warn(
							{ err: translateErr?.message || String(translateErr) },
							"Text translation fallback failed",
						);
					}
				}
			}
			...
			logger.info(
				{
					...
					preferredModel,
					activeApp,
					customTermsCount: hintEntries.length,
					dictationPreset,
					charCount: text.length,
					byteLength: audioBuffer.length,
					usedPaidKey: winner.usedPaidKey,
				},
				"Gemini STT transcribed successfully",
			);
			return { rawText: text, activeApp, usedPaidKey: winner.usedPaidKey };
		} catch (err: any) {
			logger.warn(
				{ model, preferredModel, err: err?.message || String(err) },
				"Model attempt failed, moving to next candidate",
			);
		}
	}
...
export async function transcribeDetailed(
	...
	const activeApp =
		typeof providerOrOptions === "object" && providerOrOptions.activeApp
			? providerOrOptions.activeApp
			: await getActiveAppName();

	const provider =
		typeof providerOrOptions === "string"
			? providerOrOptions
			: (providerOrOptions.provider ?? "gemini");
	const geminiModel =
		typeof providerOrOptions === "object"
			? (providerOrOptions.geminiModel ?? "gemini-3.1-flash-lite")
			: "gemini-3.1-flash-lite";
	const dictationPreset =
		typeof providerOrOptions === "object"
			? providerOrOptions.dictationPreset
			: "fast";
	const customVocabulary =
		typeof providerOrOptions === "object"
			? providerOrOptions.customVocabulary
			: [];
	const presetVocabulary =
		typeof providerOrOptions === "object"
			? providerOrOptions.presetVocabulary
			: {};
	const symbolScannerEnabled =
		typeof providerOrOptions === "object"
			? (providerOrOptions.symbolScannerEnabled ?? true)
			: true;
	const workspacePath =
		typeof providerOrOptions === "object"
			? providerOrOptions.workspacePath
			: undefined;
	const fileExtension =
		typeof providerOrOptions === "object"
			? providerOrOptions.fileExtension
			: undefined;
	const translateEnabled =
		typeof providerOrOptions === "object"
			? providerOrOptions.translateEnabled
			: undefined;
	const targetLanguage =
		typeof providerOrOptions === "object"
			? providerOrOptions.targetLanguage
			: undefined;
	const selectedText =
		typeof providerOrOptions === "object"
			? providerOrOptions.selectedText
			: undefined;
	const abortSignal =
		typeof providerOrOptions === "object"
			? providerOrOptions.abortSignal
			: undefined;
	const persistedVocabulary = loadPersistedVocabulary();
	const dictionaryEntries =
		typeof providerOrOptions === "object" &&
		providerOrOptions.dictionaryEntries !== undefined
			? providerOrOptions.dictionaryEntries
			: migrateVocabulary(
					customVocabulary || [],
					presetVocabulary || {},
					persistedVocabulary.entries || [],
					loadUserDictionary(),
				);
	...
	const rawPreset = resolveEffectivePreset(
		dictationPreset,
		activeApp,
		appPresetMappings,
	);
	...
	const isTranslationTargetingNonBurmese =
		isTranslationActive === true &&
		(effectiveTargetLang ?? "English") !== "Burmese";
	...
	if (isTranslationTargetingNonBurmese && !abortSignal?.aborted) {
		...
		if (result.success && result.finalText) {
			...
			if (BURMESE_UNICODE_REGEX.test(finalText)) {
				throw new Error(
					"Translation incomplete: Burmese script remained in transcript",
				);
			}
			return {
				text: finalText,
				usedPaidKey:
					result.translationStage?.usedPaidKey ||
					result.sourceStage.usedPaidKey ||
					false,
				modelUsed:
					result.translationStage?.modelUsed ||
					result.sourceStage.modelUsed ||
					geminiModel,
			};
		} else {
			if (
				result.sourceStage?.status === "cancelled" ||
				result.translationStage?.status === "cancelled"
			) {
	...
	const sanitized = sanitizeTranscribedText(
		rawText,
		activeApp,
		effectivePreset,
		dictionaryEntries,
	...
	if (
		isTranslationTargetingNonBurmese &&
		BURMESE_UNICODE_REGEX.test(sanitized)
	) {
		throw new Error(
			"Translation incomplete: Burmese script remained in transcript",
		);
