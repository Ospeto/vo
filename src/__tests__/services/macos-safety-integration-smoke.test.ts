...
import { SafePasteService, type ClipboardSnapshot, type SafePasteDiagnostic, type TargetIdentity } from "../../services/safe-paste.js";
import { sendCommand, validateDaemonResponse } from "../../services/daemon-ipc.js";
import {
...
const assertSanitizedDiagnosticSchema = (records: SafePasteDiagnostic[]): void => {
  const allowedKeys = new Set(["operationId", "stage", "durationMs", "targetMode", "outcome", "reason"]);
  const forbiddenKeys = new Set(["bundleId", "appName", "pid", "windowId", "windowTitle", "elementId", "role", "subrole", "args", "command", "helper"]);
...
describe.serial("macOS safety reliability integration smoke", () => {
  ...
  test("keeps a newer lifecycle safe from stale renderer acknowledgements and rejects transient toggles", () => {
    ...
    expect(lifecycle.acknowledgeStop(stop.sequenceId, true).accepted).toBe(true);
    expect(lifecycle.requestToggle().accepted).toBe(false);
  ...
  test("retains transcript without injection when target validation or conditional paste fails", async () => {
    ...
    const paste = new SafePasteService(
      () => current,
      async () => { injected++; throw new Error("conditional paste rejected"); },
      clipboard,
    );
    paste.captureTarget();
    current = target;
  ...
  test("does not settle lifecycle or apply stale paste effects after renderer teardown", async () => {
    ...
    let release!: () => void;
    const pendingPaste = new Promise<void>((resolve) => { release = resolve; });
    let injected = 0;
    const clipboard = {
      readText: () => "old clipboard",
      writeText: (_text: string) => {},
  ...
  test("settles successful paste and lifecycle only for the matching sequence", async () => {
    ...
    const clipboard = {
      ...
      restore: (snapshot: ClipboardSnapshot) => { clipboardText = snapshot.text ?? ""; },
    };
    const paste = new SafePasteService(
      () => current,
      async (expected) => { injectedTarget = expected; },
      clipboard,
      undefined,
    ...
    expect(await coordinator.pasteText("hello")).toEqual({ status: "submitted" });
    ...
    expect(records.map(({ stage }) => stage)).toEqual(["target_capture", "target_recheck", "clipboard_snapshot", "clipboard_write", "injection", "clipboard_hold", "clipboard_restore", "total"]);
    expect(records.every((record) => Number.isFinite(record.durationMs) && record.durationMs >= 0 && record.targetMode === "window" && typeof record.operationId === "string")).toBe(true);
    ...
    expect(lifecycle.finishTranscription(stop.sequenceId + 1, true).accepted).toBe(false);
    expect(lifecycle.finishTranscription(stop.sequenceId, true)).toMatchObject({ accepted: true, state: "idle" });
  });

  test.each([
    ["app change", { ...target, appName: "Other" }, "target_mismatch"],
    ["bundle ID change", { ...target, bundleId: "com.other.Editor" }, "target_mismatch"],
    ["pid change", { ...target, pid: 124 }, "target_mismatch"],
    ["window change", { ...target, windowId: 457 }, "target_mismatch"],
    ["target disappearance", null, "target_unavailable"],
  ] as const)("retains transcript when %s occurs", async (_change, current, reason) => {
    const records: SafePasteDiagnostic[] = [];
    let clipboardText = "old clipboard";
    let injected = 0;
    const clipboard = {
    ...
    expect(await paste.paste("retained transcript")).toEqual({ ok: false, reason });
    expect(injected).toBe(0);
    expect(clipboardText).toBe("old clipboard");
    expect(records.at(-1)).toMatchObject({ stage: "total", outcome: "failure", reason, targetMode: "window" });
    const serialized = JSON.stringify(records);
  ...
  test.each(["injection_rejected"] as const)("retains transcript after native injection %s", async (reason) => {
    const records: SafePasteDiagnostic[] = [];
    let clipboardText = "old clipboard";
    const clipboard = {
      readText: () => clipboardText,
    ...
    const paste = new SafePasteService(
      () => target,
      async () => { throw Object.assign(new Error("native injection failed"), { reason }); },
      clipboard,
      undefined,
      { emit: (diagnostic) => records.push(diagnostic) },
    );
    ...
    expect(records.every((record) => Number.isFinite(record.durationMs) && record.durationMs >= 0)).toBe(true);
    const serialized = JSON.stringify(records);
  ...
  ])("keeps ordinary %s targets eligible despite secure-looking field metadata", async (appName, bundleId) => {
    const fixture = { ...target, appName, bundleId, role: "AXSecureTextField", subrole: "AXPasswordField" } as TargetIdentity;
    let injected = 0;
    const clipboard = { readText: () => "old", writeText: () => {}, snapshot: () => ({ text: "old", formats: [] }), restore: () => {} };
    const paste = new SafePasteService(() => fixture, async () => { injected++; }, clipboard);
    paste.captureTarget();
    expect(await paste.paste("ordinary transcript")).toEqual({ ok: true, reason: "injection_requested" });
    expect(injected).toBe(1);
  });

  test("rejects malformed daemon responses and verifies shutdown ownership before action", async () => {
    expect(() => validateDaemonResponse({ ok: true }, "status")).toThrow("Invalid daemon status");
    expect(() => validateDaemonResponse({ ok: true, state: "idle", cwd: "/tmp", pid: 1, uptime: -1 }, "status")).toThrow();

    const socketPath = join(tmpdir(), `pi-voice-smoke-${Date.now()}-${Math.random()}.sock`);
    const server = createServer((connection) => {
      connection.on("data", (data) => {
        const request = JSON.parse(data.toString()) as { command: string; expectedPid?: number };
        if (request.command === "status") connection.write('{"ok":true}\n');
        else connection.write(JSON.stringify(request.expectedPid === 321 ? { ok: true } : { ok: false, error: "ownership mismatch" }) + "\n");
      });
    });
    servers.push(server);
    await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(socketPath, resolve); });
    await expect(sendCommand("status", socketPath, 100)).rejects.toThrow("Invalid response");
    await expect(sendCommand("shutdown", 999, socketPath, 100)).resolves.toEqual({ ok: false, error: "ownership mismatch" });
    await expect(sendCommand("shutdown", 321, socketPath, 100)).resolves.toEqual({ ok: true });
  });

  test("does not let an old runtime revision erase its replacement or discard unavailable state", () => {
    const directory = join(tmpdir(), `pi-voice-runtime-smoke-${Date.now()}-${Math.random()}`);
    ...
    expect(removeRuntimeStateIfRevision(old.revision)).toEqual({ ok: true, removed: false });
    expect(readRuntimeStateResult()).toMatchObject({ kind: "present", revision: replacement.revision, state: { cwd: "/tmp/replacement" } });

    writeFileSync(join(directory, "runtime-state.json"), JSON.stringify({
      version: 1,
    ...
    expect(readRuntimeStateResult()).toMatchObject({ kind: "present", liveness: "dead", state: { cwd: "/tmp/unavailable-daemon" } });
