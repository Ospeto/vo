...
export function parseAndValidateGuiConfig(
	input: Partial<MenuBarGuiConfig>,
): MenuBarGuiConfig {
	const defaults = getDefaultGuiConfig();

	const validModels: GeminiModelChoice[] = [
		"gemini-3.1-flash-lite",
		"gemini-2.5-flash",
	];
	const geminiModel = validModels.includes(
	...
	let inputGain =
		typeof input.inputGain === "number" && !Number.isNaN(input.inputGain)
			? input.inputGain
			: defaults.inputGain;
...
export function calculateAudioIntensity(
	samples: Float32Array,
	gain: number = 1.0,
): { rmsDb: number; percentage: number; isSilent: boolean } {
...
export function validateHotkeyCombination(hotkeyStr: string): {
	isValid: boolean;
	reason?: string;
} {
	...
	if (reserved.includes(normalized)) {
		return {
			isValid: false,
			reason: `Hotkey "${hotkeyStr}" is a reserved macOS system shortcut`,
		};
	}
	...
	if (nonModifiers.length === 0)
		return { isValid: false, reason: "No non-modifier key specified" };
	if (nonModifiers.length > 1)
		return { isValid: false, reason: "Multiple main keys specified" };
...
describe("macOS Menu Bar GUI - Production Contract Test Suite", () => {
	describe("1. Configuration Persistence & Bounds Contracts", () => {
		test("validates and accepts gemini-2.5-flash model selection", () => {
			const parsed = parseAndValidateGuiConfig({
	...
	describe("6. Dictation Presets & Custom Vocabulary Contracts", () => {
		test("accepts translate dictation preset in config and resolves prompt", () => {
			expect(
				configFileSchema.safeParse({ dictationPreset: "translate" }).success,
			).toBe(true);
			expect(getPresetPromptInstructions("translate")).toContain(
				"TRANSLATE TO ENGLISH",
			);
		});

		test("returns correct prompt instructions and temperature for each dictation preset", () => {
			expect(getPresetPromptInstructions("code_comment")).toContain(
				"SYSTEMATIC CODE DICTATION & TECHNICAL INSTRUCTION",
			);
			expect(getPresetPromptInstructions("code_comment", true)).toContain(
				"FAITHFUL TRANSLATION & ZERO IMPROVISATION",
			);
			expect(getPresetPromptInstructions("code_comment")).toContain(
				"SPOKEN IDENTIFIER FORMATTING",
			);
			expect(getPresetPromptInstructions("careful")).toContain(
				"CAREFUL DEEP PROOFREADING",
			);
		...
		test("excludes gemini-3.6-flash from fallback chain to protect free tier rate limits", () => {
			const codeChain = getFallbackModelChain(
				"gemini-3.1-flash-lite",
				"code_comment",
			);
			expect(codeChain).not.toContain("gemini-3.6-flash");

			const generalChain = getFallbackModelChain(
				"gemini-3.1-flash-lite",
				"careful",
			);
		...
		test("dynamically resolves effective preset when auto mode is selected", () => {
			...
			expect(resolveEffectivePreset("auto", "Obsidian")).toBe(
				"burmese_written",
			);
