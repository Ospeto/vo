...
describe("Coding Preset Optimization & Vibe Coding Suite (code_comment)", () => {
	describe("1. Detect Mode (translateEnabled: false)", () => {
		test("getPresetPromptInstructions returns faithful detect mode prompt preserving original language and identifier casing", () => {
			...
			expect(instructions).toContain(
				"SYSTEMATIC CODE DICTATION & TECHNICAL INSTRUCTION (DETECT MODE)",
			);
			expect(instructions).toContain(
			...
			expect(instructions).toContain(
				"ZERO CONVERSATIONAL PREAMBLES & ZERO BOILERPLATE",
			);
		});

		test("sanitizeTranscribedText preserves Burmese script losslessly when translateEnabled is false", () => {
			const input =
				"Review and refactor code, ensuring error handling branches are consolidated ပြန်တော့ review လုပ်ရဦးမလားလို့";
			const result = sanitizeTranscribedText(
				input,
				"VS Code",
				"code_comment",
				undefined,
		...
		test("sanitizeTranscribedText preserves Burmese vibe coding dictation for auto preset in coding target apps when translateEnabled is false", () => {
			...
			expect(result).toBe(
				"Add camel case user id check ဒီ function ကို refactor လုပ်မယ်",
			);
		});
	});
	...
	describe("2. Translation Mode (translateEnabled: true)", () => {
		test("getPresetPromptInstructions returns English technical specification prompt optimized for AI coding assistants", () => {
			...
			expect(instructions).toContain(
				"SYSTEMATIC CODE DICTATION & TECHNICAL INSTRUCTION (TRANSLATION MODE)",
			);
			expect(instructions).toContain(
			...
			expect(instructions).toContain(
				"STRICT ENGLISH ONLY (ZERO BURMESE SCRIPT)",
			);
		...
		test("sanitizeTranscribedText preserves residual Burmese characters on mixed lines when translateEnabled is true", () => {
			const input =
				"Refactor this function to handle errors gracefully ပြန်ကြည့်ပေးပါ";
			const result = sanitizeTranscribedText(
				input,
				"Cursor",
				"code_comment",
	...
	describe("3. Selection Transform Mode under Coding Preset", () => {
		test("resolves coding preset for coding editor apps", () => {
			expect(resolveEffectivePreset("code_comment", "Cursor")).toBe(
				"code_comment",
			);
			expect(resolveEffectivePreset("code_comment", "VS Code")).toBe(
				"code_comment",
			);
	...
	describe("4. Zero Conversational Intros & Zero Unrequested Code Block Hallucinations", () => {
		test("sanitizer strips wrapping double, single, and curly quotes added by LLMs", () => {
			...
			).toBe("Return early if userId is null");
			expect(
				sanitizeTranscribedText(
					"'const userId = 123;'",
					"Cursor",
			...
			).toBe("Check user permissions");
		});

		test("both detect and translate prompt instructions enforce zero preambles and zero boilerplate directives", () => {
			const detectInstructions = getPresetPromptInstructions(
				"code_comment",
				false,
			);
			const translateInstructions = getPresetPromptInstructions(
				"code_comment",
				true,
			);

			expect(detectInstructions).toContain(
				"ZERO CONVERSATIONAL PREAMBLES & ZERO BOILERPLATE",
			);
			expect(translateInstructions).toContain(
				"ZERO BOILERPLATE & ZERO PREAMBLES",
			);
		});
	});
	...
	describe("5. Spoken Casing Transforms & Language-Aware Comment Syntax", () => {
		test("sanitizeCodePresetText converts spoken casing commands and strips preambles", () => {
			const input =
				"Here is the specification: Check camel case user response and snake case created at";
		...
		test("sanitizeCodePresetText handles hyphenated input in casing commands", () => {
			expect(sanitizeCodePresetText("kebab case user-card")).toBe(
				"`user-card`",
			);
			expect(sanitizeCodePresetText("camel case user-card")).toBe("`userCard`");
			expect(sanitizeCodePresetText("snake case user-card")).toBe(
				"`user_card`",
			);
			expect(
				sanitizeCodePresetText("kebab case user-card should be used"),
			).toBe("`user-card` should be used");
		});
