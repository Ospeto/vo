...
- Final `bun audit --json`: expected exit 1 because advisories remain; before/after inventory is 101 -> 57 rows and the reachable `protobufjs`, `@protobufjs/utf8`, `undici`, and `ws` rows are absent. The intermediate 40.8.3 audit is retained separately; final Electron 40.8.5 has no Electron audit rows (`audit-review.md`, `audit-final.json`).
